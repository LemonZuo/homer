...
// ParseEsxiTarget 把一行 esxi_host 翻成 sshlike.Target。
// sshlike 包不依赖 internal/model,翻译留在 esximon 模块自己做。
func ParseEsxiTarget(h model.EsxiHost) (*sshlike.Target, error) {
	...
	host, port, err := sshlike.SplitEndpoint(h.Endpoint, "ESXi")
	if err != nil {
		return nil, err
	}
	out := &sshlike.Target{
...
// LoadEsxiBastion 加载一台被引用作为跳板机的 esxi_host,校验启用状态。
func LoadEsxiBastion(db *gorm.DB, id int64) (*sshlike.Target, error) {
	if db == nil {
	...
	return ParseEsxiTarget(row)
}

// FindEsxiUpstream 反查是否有别的 esxi_host 把 id 当作 bastion 在用。
func FindEsxiUpstream(db *gorm.DB, id int64) (string, bool, error) {
	if db == nil {
