...
// List 返回所有凭证;RefCount 为引用此凭证的 esxi_host 数。
func (s *CredentialStore) List() ([]model.EsxiSSHCredential, error) {
	var rows []model.EsxiSSHCredential
...
func (s *CredentialStore) Get(id int64) (*model.EsxiSSHCredential, error) {
	...
	var row model.EsxiSSHCredential
...
// Upsert id=0 创建,否则更新。
func (s *CredentialStore) Upsert(c *model.EsxiSSHCredential) (*model.EsxiSSHCredential, error) {
	if c == nil {
	...
	var existing model.EsxiSSHCredential
...
// Delete 删除前校验是否被任何 esxi_host 引用。
func (s *CredentialStore) Delete(id int64) error {
	...
	res := s.db.Delete(&model.EsxiSSHCredential{}, id)
	if res.Error != nil {
		return res.Error
	}
...
func normalizeCredential(c *model.EsxiSSHCredential) {
...
func validateCredential(c model.EsxiSSHCredential) error {
	if c.Name == "" {
