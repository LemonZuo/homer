...
type SeriesPoint struct {
	...
	OutputVoltage float32   `json:"output_voltage"` // V,桶内 avg
...
//
// 聚合规则:
//   - input_voltage / output_voltage: AVG()(电压一般波动小,平均能反映电网情况)
...
func (s *Store) Series(hostKind string, hostID int64, upsName string, since time.Time, bucketSec int) ([]SeriesPoint, error) {
	...
	err := s.db.Model(&model.UPSSample{}).
		Select(bucketExpr+" AS bucket_time, AVG(CASE WHEN input_voltage>=0 THEN input_voltage ELSE NULL END) AS avg_input_v, AVG(CASE WHEN output_voltage>=0 THEN output_voltage ELSE NULL END) AS avg_output_v, MAX(CASE WHEN load_percent>=0 THEN load_percent ELSE NULL END) AS max_load, MAX(CASE WHEN real_power>=0 THEN real_power ELSE NULL END) AS max_real_pow, SUM(CASE WHEN power_source='low_battery' THEN 1 ELSE 0 END) AS has_lb, SUM(CASE WHEN power_source='battery' THEN 1 ELSE 0 END) AS has_ob, SUM(CASE WHEN power_source='mains' THEN 1 ELSE 0 END) AS has_ol").
