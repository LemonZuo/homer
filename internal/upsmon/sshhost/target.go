// Package sshhost 是 UPS 模块自带的 SSH 连接语义:
// auth_json/config_json 解析、凭证模式、单跳跳板校验、sshx.Conn 构造。
// 跟 acme/deployer/sshlike 同形但完全独立,不依赖 internal/acme。
package sshhost
...
// TargetAuth ups_host.auth_json 的反序列化形态。
...
// TargetConfig ups_host.config_json 的反序列化形态。
type TargetConfig struct {
	BastionHostID int64 `json:"bastion_host_id,omitempty"`
}

// Target 解析后扁平化的目标,便于后续 ConnFor / ResolveCredential 操作。
type Target struct {
	ID            int64
	Name          string
	Host          string
...
// ParseTarget 把一行 ups_host 解析成 Target。
func ParseTarget(h model.UPSHost) (*Target, error) {
	auth := TargetAuth{}
...
func SplitEndpoint(endpoint string) (string, int, error) {
	...
	if endpoint == "" {
		return "", 0, errors.New("UPS 主机不能为空")
	}
	host, portText, err := net.SplitHostPort(endpoint)
	if err == nil {
		port, err := strconv.Atoi(portText)
		if err != nil {
			return "", 0, errors.New("UPS 端口无效")
		}
...
func ValidateTarget(t Target) error {
	...
	if t.Host == "" {
		return errors.New("UPS 主机不能为空")
	}
	if t.Port <= 0 || t.Port > 65535 {
		return errors.New("UPS 端口无效")
	}
