...
// Store 在 internal/upsmon 包里实现,这里只依赖 interface,避免双向 import。
type CredentialResolver interface {
	Get(id int64) (*model.UPSSSHCredential, error)
}
...
func ResolveCredential(resolver CredentialResolver, t *Target) error {
	...
	if resolver == nil {
		return errors.New("凭证模式未注入 UPS SSH 凭证库")
	}
	if t.CredentialID <= 0 {
	...
	if err != nil {
		return fmt.Errorf("加载 UPS SSH 登录凭证失败:%w", err)
	}
