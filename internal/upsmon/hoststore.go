...
var ErrHostNotFound = errors.New("UPS 主机不存在")

// HostStore ups_host 表的 CRUD。
// 校验委托给 sshlike.ValidateTarget / ValidateBastion,保持单一真相。
...
// List 按 id 升序返回所有 UPS 主机。
func (s *HostStore) List() ([]model.UPSHost, error) {
...
func (s *HostStore) ListEnabled() ([]model.UPSHost, error) {
	var rows []model.UPSHost
...
// Get 按 id 取一条。
func (s *HostStore) Get(id int64) (*model.UPSHost, error) {
	if id <= 0 {
...
// Upsert 创建或按 id 更新。
func (s *HostStore) Upsert(in HostInput) (*model.UPSHost, error) {
	in.Name = strings.TrimSpace(in.Name)
	in.Endpoint = strings.TrimSpace(in.Endpoint)
	host, port, err := sshlike.SplitEndpoint(in.Endpoint, "UPS")
	if err != nil {
		return nil, err
	}
	t := sshlike.Target{
	...
	sshlike.Normalize(&t)
	if err := sshlike.ValidateTarget(t, "UPS"); err != nil {
		return nil, err
	}
	loader := func(id int64) (*sshlike.Target, error) { return LoadUPSBastion(s.db, id) }
	finder := func(id int64) (string, bool, error) { return FindUPSUpstream(s.db, id) }
...
func (s *HostStore) SetEnabled(id int64, enabled bool) error {
	...
	res := s.db.Model(&model.UPSHost{}).Where("id = ?", id).Update("enabled", model.BoolFlag(enabled))
	if res.Error != nil {
		return res.Error
	}
...
func (s *HostStore) Delete(id int64) error {
	...
	name, ok, err := FindUPSUpstream(s.db, id)
	if err != nil {
		return err
	}
	if ok {
	...
	res := s.db.Delete(&model.UPSHost{}, id)
	if res.Error != nil {
		return res.Error
	}
	if res.RowsAffected == 0 {
...
// CredentialUsage 统计每条 ups_ssh_credential 被多少台 ups_host 引用,
// CredentialStore.List 时用。
...
// HostsByCredential 列出引用了指定凭证的所有主机名(凭证删除前校验用)。
