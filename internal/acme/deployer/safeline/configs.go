...
var ErrDeployConfigNotConfigured = errors.New("雷池部署配置未配置")
...
func (s *DeployConfigStore) ListByDomain(domainID int64) ([]model.ACMESafelineDeployConfig, error) {
	rows, err := s.configs.ListByDomain(domainID, acme.DeployKindSafeline)
	if err != nil {
		return nil, err
	}
...
func (s *DeployConfigStore) ListAutoByDomain(domainID int64) ([]model.ACMESafelineDeployConfig, error) {
	rows, err := s.configs.ListAutoByDomain(domainID, acme.DeployKindSafeline)
	if err != nil {
		return nil, err
	}
	out := make([]model.ACMESafelineDeployConfig, 0, len(rows))
...
func (s *DeployConfigStore) Get(id int64) (*model.ACMESafelineDeployConfig, error) {
	row, err := s.configs.Get(id)
	if err != nil {
		return nil, fmt.Errorf("%w: %v", ErrDeployConfigNotConfigured, err)
	}
	if row.Kind != acme.DeployKindSafeline {
		return nil, fmt.Errorf("%w: id=%d 类型不是雷池", ErrDeployConfigNotConfigured, id)
	}
	cfg := deployConfigFromGenericConfig(*row)
	return &cfg, nil
}

func (s *DeployConfigStore) Upsert(domainID int64, c *model.ACMESafelineDeployConfig) (*model.ACMESafelineDeployConfig, error) {
	if c == nil {
