...
var ErrTargetNotConfigured = errors.New("雷池实例未配置")
...
func (s *TargetStore) List() ([]model.ACMESafelineTarget, error) {
	rows, err := s.targets.List(acme.DeployKindSafeline)
	if err != nil {
		return nil, err
	}
...
func (s *TargetStore) Get(id int64) (*model.ACMESafelineTarget, error) {
	row, err := s.targets.Get(id)
	if err != nil {
		return nil, fmt.Errorf("%w: %v", ErrTargetNotConfigured, err)
	}
	if row.Kind != acme.DeployKindSafeline {
		return nil, fmt.Errorf("%w: id=%d 类型不是雷池", ErrTargetNotConfigured, id)
	}
	return targetFromDeployTarget(*row)
}

func (s *TargetStore) Upsert(t *model.ACMESafelineTarget) (*model.ACMESafelineTarget, error) {
	if t == nil {
