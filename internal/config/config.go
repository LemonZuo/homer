...
type Config struct {
	...
	// 路由器维护窗口。日内 HH:MM-HH:MM(本地时区),留空 / off / none / disabled 表示禁用。
	// 窗口内跳过 UPS / ESXi 的 cron 采样,避免路由器定时重启造成的离线误报;
	// 命名以「维护主体」为准:被维护的是路由器,不是 SSH 监控本身。手动触发不受影响。
	RouterMaintenanceWindow string
	...
	EsxiTopologyRefreshIntervalMin int
...
func Load() *Config {
	...
	return &Config{
		...
		RouterMaintenanceWindow: env("ROUTER_MAINTENANCE_WINDOW", ""),
		...
		EsxiTopologyRefreshIntervalMin: envInt("ESXI_TOPOLOGY_REFRESH_INTERVAL_MIN", 30),
