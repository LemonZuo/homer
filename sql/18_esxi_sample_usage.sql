-- ESXi 采样表补 3 个使用率/容量明细列，给历史曲线画「CPU 使用率 / 内存已用 / 各盘已用」用：
--   cpu_usage_percent     SMALLINT  -- -1=无数据
--   memory_usage_json     TEXT      -- {"used_bytes":N,"total_bytes":N,"usage_percent":N}
--   disk_usage_json       TEXT      -- [{"device":"naa.xxx","used_bytes":N,"capacity_bytes":N}, ...]
-- 旧行三列分别用 -1 / NULL / NULL 兜底,前端按已有约定当「无数据」处理。
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM INFORMATION_SCHEMA.COLUMNS
    ...
    ALTER TABLE `esxi_sample`
      ...
      ADD COLUMN `memory_usage_json`    TEXT NULL COMMENT '内存使用明细' AFTER `cpu_usage_percent`;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM INFORMATION_SCHEMA.COLUMNS
                 WHERE TABLE_SCHEMA = DATABASE()
                   AND TABLE_NAME = 'esxi_sample'
                   AND COLUMN_NAME = 'memory_usage_json') THEN
    ALTER TABLE `esxi_sample`
      ADD COLUMN `memory_usage_json` TEXT NULL COMMENT '内存使用明细' AFTER `cpu_usage_percent`;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM INFORMATION_SCHEMA.COLUMNS
