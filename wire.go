...
func buildEsxiService(gormDB *gorm.DB, cfg *config.Config, hub *notify.Hub) (*esximon.Service, *esximon.Sampler, *esximon.HostStore, *esximon.CredentialStore) {
	...
	sampler := esximon.NewSampler(
		gormDB,
		hosts,
		creds,
		time.Duration(cfg.EsxiSSHTimeoutSec)*time.Second,
		time.Duration(cfg.EsxiSlowRefreshIntervalMin)*time.Minute,
		time.Duration(cfg.EsxiCommandSlowLogMS)*time.Millisecond,
	)
