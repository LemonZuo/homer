...
export function UpsCredentialsDrawer({
  open,
  onOpenChange,
  credentials,
  onAdd,
  ...
  onAdd: () => void
  onEdit: (c: UpsCredential) => void
  onDelete: (c: UpsCredential) => void
}) {
  return (
    <Drawer open={open} onOpenChange={onOpenChange}>
      <DrawerContent>
        <DrawerHeader>
          <DrawerTitle>UPS 登录凭证</DrawerTitle>
          <DrawerDescription>
            UPS 模块专属 SSH 凭证库;一份登录身份可被多台 UPS 机器共用
          </DrawerDescription>
        </DrawerHeader>
