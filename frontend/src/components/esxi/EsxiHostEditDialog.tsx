...
import type { EsxiCredential, EsxiHost, EsxiHostInput } from './types'
...
export function EsxiHostEditDialog({
  open,
  onOpenChange,
  target,
  hosts,
  ...
  target: EsxiHost | null
  hosts: EsxiHost[]
  credentials: EsxiCredential[]
  ...
  const save = async () => {
    ...
    try {
      if (target?.id) {
        await api.put(`/esxi/hosts/${target.id}`, payload)
      } else {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90dvh] w-[calc(100%-2rem)] overflow-y-auto p-4 sm:p-6">
        <DialogHeader>
          <DialogTitle>{target ? '编辑 ESXi 机器' : '新增 ESXi 机器'}</DialogTitle>
          <DialogDescription>
            通过 SSH 拨号执行 esxcli / vsish / vim-cmd 拉取平台、CPU 温度、磁盘 SMART 等
          </DialogDescription>
        </DialogHeader>
        <div className="grid gap-3.5">
          <div className="grid gap-1.5">
            <Label htmlFor="esxi-name">机器名称</Label>
            <Input
              id="esxi-name"
              value={name}
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-3 sm:gap-2">
            <div className="grid gap-1.5 sm:col-span-2">
              <Label htmlFor="esxi-host">主机</Label>
              <Input
                id="esxi-host"
                value={host}
            ...
            <div className="grid gap-1.5">
              <Label htmlFor="esxi-port">端口</Label>
              <Input
                id="esxi-port"
                value={port}
          ...
          {authMode === 'credential' ? (
            <div className="grid gap-1.5">
              <div className="flex items-center justify-between">
                <Label htmlFor="esxi-credential">登录凭证</Label>
                <button
                  type="button"
            ...
            <div className="grid gap-1.5">
              <Label htmlFor="esxi-user">用户名</Label>
              <Input
                id="esxi-user"
                value={username}
          ...
          {authMode === 'password' && (
            <div className="grid gap-1.5">
              <Label htmlFor="esxi-password">密码</Label>
              <Input
                id="esxi-password"
                type="password"
                value={password}
          ...
          {authMode === 'key' && (
            <>
              <div className="grid gap-1.5">
                <Label htmlFor="esxi-private-key">私钥</Label>
                <Textarea
                  id="esxi-private-key"
                  value={privateKey}
              ...
              <div className="grid gap-1.5">
                <Label htmlFor="esxi-passphrase">私钥口令(可选)</Label>
                <Input
                  id="esxi-passphrase"
                  type="password"
                  value={passphrase}
          ...
          <div className="grid gap-1.5">
            <Label htmlFor="esxi-bastion">跳板机(可选)</Label>
            <Select
              id="esxi-bastion"
          ...
          <div className="flex items-center justify-between">
            <Label htmlFor="esxi-enabled">启用</Label>
            <Switch id="esxi-enabled" checked={enabled} onChange={(v) => setEnabled(v)} />
          </div>
        </div>
